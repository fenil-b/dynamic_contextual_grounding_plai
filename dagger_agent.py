...
from random_base_agent import BaseAgent
import alfworld.agents.modules.memory as memory
from alfworld.agents.modules.generic import to_np, to_pt, pad_sequences, max_len, BeamSearchNode
from alfworld.agents.modules.layers import NegativeLogLoss, compute_mask
...
class TextDAggerAgent(BaseAgent):
    ...
    def choose_softmax_action(self, action_rank, action_mask=None):
        action_rank = action_rank - torch.min(action_rank, -1, keepdim=True)[0] + 1e-3
        ...
        with torch.no_grad():
            ...
            for b in range(batch_size):
                ...
                while(True):
                    ...
                    epsilon = torch.le(pred, 0.0).float() * 1e-9
