...
from alfworld.agents.modules.generic import to_pt, pad_sequences, preproc, max_len, LinearSchedule
...
class BaseAgent:
    ...
    def __init__(self, config):
        ...
        gpt2_model = GPT2Model.from_pretrained('gpt-2')
        gpt2_model.transformer = None
        gpt2_model.encoder = None
        for param in gpt2_model.parameters():
            param.requires_grad = False

        self.online_net = Policy(config=self.config, gpt2_model=gpt2_model, word_vocab_size=len(self.word2id))
        self.target_net = Policy(config=self.config, gpt2_model=gpt2_model, word_vocab_size=len(self.word2id))
